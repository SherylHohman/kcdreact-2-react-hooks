...
TODO: submit PR re: clarification that 06.extra-4.js is only replacing UI, via
error boundary. App functionality is not restored. That will be covered in the
following extra credit (06.extra-4.js)
...
Don't worry that your app does not retain functionality after your error
boundary component handles an error. We will cover that in the next extra
credit. Here we are just swapping out a broken UI so the app does not crash to a
